...
/**
 * This is an event sourced entity. It has a state, {@link YoutubeState}, which
 ...
 */
public class YoutubeEntity extends PersistentEntity<YoutubeCommand, YoutubeEvent, YoutubeState> {
    /**
     * An entity can define different behaviours for different states, but it will
     * always start with an initial behaviour. This entity only has one behaviour.
     */
    @Override
    public Behavior initialBehavior(Optional<YoutubeState> snapshotState) {
        /*
        ...
        BehaviorBuilder b = newBehaviorBuilder(
                snapshotState.orElse(new YoutubeState("Hello", LocalDateTime.now().toString()))
        );
        ...
        b.setEventHandler(GreetingMessageChanged.class,
                ...
                // the event.
                evt -> new YoutubeState(evt.message, LocalDateTime.now().toString())
        );
